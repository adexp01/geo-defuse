...
const Card: FC<Props> = ({ label, icon: Icon, onClick }) => {
  return (
    <button
      onClick={onClick}
      className="relative bg-[#f8dfc1] hover:bg-[#C8A67D] rounded-[20px] flex items-center justify-center h-[230px] flex-1 text-[#C8A67D] hover:text-[#FFF8EB] group transition-all duration-300"
    >
      <div className="absolute top-5 left-5 w-[100px] object-cover">
        <Icon fill={"currentColor"} />
      </div>
      <p className="text-center text-[#373737] text-2xl max-lg:text-xl font-normal max-w-[50%] group-hover:text-[#FFF8EB]">
        {label}
      </p>
    </button>
  );
